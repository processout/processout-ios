...
final class MockWebAuthenticationSession: POWebAuthenticationSession {

    var authenticateCallsCount: Int {
        lock.withLock { _authenticateCallsCount }
    }

    var authenticateFromClosure: ((POWebAuthenticationRequest) async throws -> URL)! {
    ...
    private nonisolated(unsafe) var _authenticateFromClosure: ((POWebAuthenticationRequest) async throws -> URL)!
}
