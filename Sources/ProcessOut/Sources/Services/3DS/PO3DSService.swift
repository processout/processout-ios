...
public protocol PO3DSService: AnyObject, Sendable {
    ...
    func handle(challenge: PO3DS2Challenge, completion: @escaping @Sendable (Result<Bool, POFailure>) -> Void)
    ...
    func handle(redirect: PO3DSRedirect, completion: @escaping @Sendable (Result<String, POFailure>) -> Void)
}
...
extension PO3DSService {
    ...
    func authenticationRequest(configuration: PO3DS2Configuration) async throws -> PO3DS2AuthenticationRequest {
        try await withUnsafeThrowingContinuation { continuation in
            authenticationRequest(configuration: configuration) { result in
    ...
    func handle(challenge: PO3DS2Challenge) async throws -> Bool {
        try await withUnsafeThrowingContinuation { continuation in
            handle(challenge: challenge) { result in
                continuation.resume(with: result)
            }
        }
    }
    ...
    func handle(redirect: PO3DSRedirect) async throws -> String {
        try await withUnsafeThrowingContinuation { continuation in
            handle(redirect: redirect) { result in
                continuation.resume(with: result)
            }
        }
    }
