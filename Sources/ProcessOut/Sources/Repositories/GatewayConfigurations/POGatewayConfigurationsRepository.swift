...
public protocol POGatewayConfigurationsRepository: PORepository { // sourcery: AutoCompletion
    ...
    func all(request: POAllGatewayConfigurationsRequest) async throws(POFailure) -> POAllGatewayConfigurationsResponse

    /// Searches configuration with given request.
    func find(request: POFindGatewayConfigurationRequest) async throws(POFailure) -> POGatewayConfiguration
}

extension POGatewayConfigurationsRepository {

    /// Returns available gateway configurations.
    public func all() async throws(POFailure) -> POAllGatewayConfigurationsResponse {
